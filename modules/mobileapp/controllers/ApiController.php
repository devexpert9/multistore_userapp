...
{	
	...
	{				
		...
		$continue=true;
		if($action=="getLanguageSettings" || $action=="registerMobile" || $action=="UploadProfile"){
	   	   $continue=false;
	    }
	    if($continue){		    	
	...
	{		
		...
		$data=array(); $total_records=0; $stmt=''; $location_type = '';
		...
		switch ($search_mode) {
			case "postcode": /* SEARCH BY LOCATION*/
			   ...
			   $query
	           $and
			   ORDER BY is_sponsored DESC, ratings DESC
			   LIMIT $limit_page
			   ";
	...
	{	
		...
		if ( $data = AddonMobileApp::merchantInformation($this->data['merchant_id'])){				
			...
			if(empty($data['coordinates']['latitude'])){
				$country=Yii::app()->functions->getAdminCountrySet();
				if( $lat_res=Yii::app()->functions->geodecodeAddress($country)){									
					$data['coordinates']['latitude'] = $lat_res['lat'];
					$data['coordinates']['longtitude'] = $lat_res['long'];
				}		
			}
	...
	{				
		...
		if ($res=Yii::app()->functions->getItemByCategory($this->data['cat_id'],false,$this->data['merchant_id'])){
			...
			foreach ($res as $val) {		
				...
				$price='';	
				if (is_array($val['prices'])  && count( (array) $val['prices'])>=1){
			...
			if (is_array($category_info) && count( (array) $category_info)>=1){
	...
	{		
		...
		if ( $res=Yii::app()->functions->getItemById($this->data['item_id'])){			
			...
			if (is_array($data['prices']) && count( (array) $data['prices'])){
				$data['has_price']=2;		
				$price=array();		
			...
			if (is_array($data['addon_item']) && count( (array) $data['addon_item'])>=1){
				$addon_item=array();					
				foreach ($data['addon_item'] as $val) {
					...
					$sub_item=array();
					if(is_array($val['sub_item']) && count( (array) $val['sub_item'])>=1){				       
	...
	{				
	    ...
	    if (is_array($merchant_info) && count( (array) $merchant_info)>=1){
		...
		if(!empty($cart)){
			...
			$new_cart = array(); $x=0;
			if(is_array($cart) && count( (array) $cart)>=1){
			   foreach ($cart as $keycart=>$cart_val) {
			   	    //dump("key=>".$keycart);
			   	    if (is_array($new_cart) && count( (array) $new_cart)>=1){
			   	    	...
			   	    	foreach ($new_cart as $key_cart=>$val_cart) {		
			   	    		...
			   	    		/*CHECK COOKING REF*/
			   	    		if(is_array($cart_val['cooking_ref']) && count( (array) $cart_val['cooking_ref'])>=1){
			   	    		   if(is_array($val_cart['cooking_ref']) && count( (array) $val_cart['cooking_ref'])>=1){
			   	    			   if($cart_val['cooking_ref'][0]['value']==$val_cart['cooking_ref'][0]['value']){
			   	    			   	  $s_cookingref=true;
			   	    			   }
			   	    		   }
			   	    		} else {
			   	    		   $c_cookingref_1=count( (array) $cart_val['cooking_ref']);	
			   	    		   $c_cookingref_2=count( (array) $val_cart['cooking_ref']);	
			   	    		...
			   	    		/*CHECK INGREDIENTS*/
			   	    		if (is_array($cart_val['ingredients']) && count( (array) $cart_val['ingredients'])>=1){
			   	    			if (is_array($val_cart['ingredients']) && count( (array) $val_cart['ingredients'])>=1){
			   	    				$i_1 = count( (array) $cart_val['ingredients']); $i_2 = count( (array) $val_cart['ingredients']);
			   	    				//dump($i_1); dump($i_2);
			   	    				if ( $i_1 == $i_2){			   	    					
			   	    					$flag_ingredients = 0;
			   	    		   ...
			   	    		   $c_ingredients_1=count( (array) $cart_val['ingredients']);	
			   	    		   $c_ingredients_2=count( (array) $val_cart['ingredients']);	
			   	    		...
			   	    		/*CHECK ADDON*/			   	    		
			   	    		if (is_array($cart_val['sub_item']) && count( (array) $cart_val['sub_item'])>=1){
			   	    			if (is_array($val_cart['sub_item']) && count( (array) $val_cart['sub_item'])>=1){
			   	    				$sub_1 = count( (array) $cart_val['sub_item']); $sub_2 = count( (array) $val_cart['sub_item']);
			   	    				if ($sub_1 == $sub_2){
			   	    					$flag_addon = 0;			   	    					
			   	    		   ...
			   	    		   $c_addon_1=count( (array) $cart_val['sub_item']);	
			   	    		   $c_addon_2=count( (array) $val_cart['sub_item']);	
			...
			$packagin_wise_fee = 0;
			$merchant_packaging_wise = getOption($mtid,'merchant_packaging_wise');		
			$merchant_packaging_increment = getOption($mtid,'merchant_packaging_increment');
			...
			if (is_array($cart) && count( (array) $cart)>=1){
			    foreach ($cart as $val) {
			    	...
			    	if($has_pts==2){
			    		...
			    		if(is_array($set_price) && count( (array) $set_price)>=1){			    			
			    			$discount_less = is_numeric($val['discount'])?$val['discount']:0;
			    			$set_price= (float)$set_price[0] - (float)$discount_less;
			    		} else $set_price=0;
			    	...
			    	if($merchant_packaging_wise==1){
			    		if($merchant_packaging_increment==2){
			    			$packagin_wise_fee+=$food['packaging_fee']*$val['qty'];
			    		} else {
			    			$packagin_wise_fee+=$food['packaging_fee'];
			    		}			    				    		
			    	}
			    	...
			    	if ($val['discount']>0){
			    		...
			    		$subtotal+=($val['qty']* (float) $item_price);
			    		$subtotal_temp = ($val['qty']* (float) $item_price);
			    	}			    
			    			...
			    			foreach ($valsub0 as $valsub) {				    				
				    			...
				    			if(!empty($valsub['value'])){
				    				...
				    				if($food['two_flavors']!=2){				    					
					    				...
					    				if(is_array($t_two_flavors_position) && count( (array) $t_two_flavors_position)>=1){
			    	...
			    	$cart_content[]=array(			    	  
			    	  ...
			    	  'total'=>$val['qty']*((float)$item_price- (float)$discount_amt),
			    	  'total_pretty'=>AddonMobileApp::prettyPrice($val['qty']* ((float)$item_price-(float)$discount_amt) ),
			    	  'size'=>$item_size,			
               ...
               /*get merchant offers*/
		        ...
		        /*packaging*/		        		        
		        if ($merchant_packaging_wise==1){
		        	$merchant_packaging_charge = $packagin_wise_fee;
		        } else {			
	           ...
	           /*get the tax*/
				...
				switch ( $this->data['transaction_type']) {
					case "delivery":						
					    ...
					    /*CHECK MIN DELIVERY BY DISTANCE*/ 
					    if ($search_mode!="postcode"){
					    	if(is_array($distance_new) && count( (array) $distance_new)>=1){					    	   
					    		$min_fees=FunctionsV3::getMinOrderByTableRates($mtid,
			    ...
			    if($res_date = FunctionsV3::getDateList($mtid)){
			    	if(is_array($res_date) && count( (array) $res_date)>=1){
			    ...
			    /*ENABLED ESTIMATION ADD TO CURRENT TIME*/
			    $enabled_estimation = false;
			    if($enabled_estimation){
				    ...
				    if(is_numeric($estimation)){
					    if($estimation>0){
					       $new_time = date('h:i A', strtotime("+$estimation hour"));
					   	   $delivery_options['default_value_time']=array(
						      'time'=>$new_time,
						      'time_pretty'=>FunctionsV3::prettyTime($new_time),
						    );			    
					    }			
				    }
			    } 
			    ...
			    /*ENABLED ESTIMATION TIME AND DATE*/
			    $enabled_auto_time_suggestion = false;
			    if($enabled_auto_time_suggestion){
				    ...
				    if(is_numeric($estimation)){
					    if($estimation>0){				
					       ...
					       $full_estimation_delivery = date('Y-m-d h:i A', strtotime("+$estimation hour"));
					       $estimation_delivery_time = date('h:i A', strtotime("+$estimation hour"));
					       $estimation_delivery_date = date('Y-m-d', strtotime("+$estimation hour"));
					       
					   	   $this->details['estimation_delivery_time'] = $estimation_delivery_time;				   	   
					   	   $day = strtolower(date("D",strtotime($full_estimation_delivery)));		
					   	   
					   	   if(isset($_GET['debug'])){
						   	   dump("current_date => $current_date");				 
						   	   dump("current_date => $estimation_delivery_date");				 				   	     	   
						   	   dump("full_estimation_delivery => $full_estimation_delivery");				   	   
						   	   dump("estimation_delivery_time=> $estimation_delivery_time");
						   	   dump("day => $day");		
					   	   }
					   	   
					   	   if($current_date!=$estimation_delivery_date){
					   	   	  if(isset($_GET['debug'])){
					   	   	  	dump("<h2>Date not the same</h2>");
					   	   	  }
					   	   	  $this->details['estimation_delivery_date'] = $estimation_delivery_date;
					   	   	  if($next_time = AddonMobileApp::getNextDeliveryTime($mtid , $day , $full_estimation_delivery)){				   	   	  	 				   	   	  	
					   	   	  	 $next_time_full = $next_time['date']." ".$next_time['time'];
					   	   	  	 /*dump($full_estimation_delivery);
					   	   	  	 dump($next_time_full);*/
					   	   	  	 
					   	   	  	 $date_diff = FunctionsV3::dateDifference($next_time_full,$full_estimation_delivery);
					   	   	  	 //dump($date_diff);
					   	   	  	 if(is_array($date_diff) && count( (array) $date_diff)>=1){
					   	   	  	 	// do nothing
					   	   	  	 } else {				   	   	  				   	   	  	 
						   	   	  	 $this->details['estimation_delivery_time'] = $next_time['time'];
							   	   	 $this->details['estimation_delivery_date'] = $next_time['date'];
					   	   	  	 }
					   	   	  }
					   	   } else {
					   	   	  if(isset($_GET['debug'])){
					   	   	  	dump("<h2>Same date</h2>");				   	   	  
					   	   	  }
					   	   	  if ( !Yii::app()->functions->isMerchantOpenTimes($mtid,$day,$estimation_delivery_time)){
					   	   	  	  //dump("<h2>Close</h2>");	
					   	   	  	  				   	   	  	  
					   	   	  	  $has_early_hours = false;
					   	   	  	  if($early_hours= FunctionsV3::getEarlyOpeningHours($mtid,$day,$full_estimation_delivery)){				   	   	  	  	 
					   	   	  	  	 $early_hours_time = $early_hours['date']." ".$early_hours['time'];				   	   	  	  	 
					   	   	  	  	 $date_diff_early = FunctionsV3::dateDifference($full_estimation_delivery,$early_hours_time);				   	   	  	  	 
					   	   	  	  	 if(is_array($date_diff_early) && count( (array) $date_diff_early)>=1){
					   	   	  	  	 	$this->details['estimation_delivery_time'] = $early_hours['time'];
						   	   	  	    $this->details['estimation_delivery_date'] = $early_hours['date'];
						   	   	  	    $has_early_hours = true;
					   	   	  	  	 }				   	   	  	  
					   	   	  	  }				   	   	  
					   	   	  	  
					   	   	  	  if($has_early_hours==false){
						   	   	  	  $full_estimation_delivery = date('Y-m-d h:i A', strtotime(" +1 day"));
						   	   	  	  $estimation_delivery_time = date('h:i A', strtotime($full_estimation_delivery));
						   	   	  	  $day = strtolower(date("D",strtotime($full_estimation_delivery)));		
						   	   	  	  /*dump("full_estimation_delivery => $full_estimation_delivery");	
						   	   	  	  dump("day => $day");*/
						   	   	  	  if($next_time = AddonMobileApp::getNextDeliveryTime($mtid , $day , $full_estimation_delivery)){				   	   	  	  	  
				    ...
				    /*dump("<h2>Resp</h2>");
				    dump($this->details['estimation_delivery_time']);
				    dump($this->details['estimation_delivery_date']);
				    die();*/
			    	
			    } /*END auto time suggestion*/	
			    /*END ENABLED ESTIMATION TIME AND DATE*/
	...
	public function actionCheckOut()
	{
       ...
       if ( isset($this->data['delivery_time'])){
       	  if(!empty($this->data['delivery_time'])){
       	  	 ...
       	  	 if (is_array($time_diff) && count( (array) $time_diff)>=1){
	   ...
	   if(AddonMobileApp::isOrderSmsEnabled($mtid)){
	   	  $order_sms_verification=1;
	   }
	   ...
	   $this->msg=array(
	     ...
	     'next_step'=>$next_step_orig,
	     'merchant_id'=>$mtid,
	     'order_sms_verification'=>$order_sms_verification
	   );
	...
	{		
		...
		$mobile_payment = FunctionsV3::PaymentOptionList();
		$mobile_payment = array_flip($mobile_payment);				
		...
		if(is_array($payment_list) && count( (array) $payment_list)>=1){
					    ...
					    case "rzr":		
					   	...
					   	case "mercadopago":   
					   	  if ( $credentials =  mercadopagoWrapper::getCredentials($mtid)){						   	  	    			   	    
					   	    	$label = $payment_list[$val];
					   	    	if($credentials['card_fee']>=0.001){
					   	    		$label =  Yii::t("default","Mercapado (card fee [fee])",array(
					   	...
					   	case "paypal_v2":   
					   	    if ( $credentials =  PaypalWrapper::getCredentials($mtid)){					   	    	
					   	    	$label = $payment_list[$val];
					   	    	if($credentials['card_fee']>=1){
					   	    		$label =  Yii::t("default","Paypal V2 (card fee [fee])",array(
					         		  '[fee]'=>FunctionsV3::prettyPrice($credentials['card_fee'])
					         		));
					   	    	}					   	    
					   	    	$merchant_payment_list[]=array(
					   	...
					   	case "stp":					   		
					   		 if ( $credentials =  StripeWrapper::getCredentials($mtid)){					   	    	
					   	    	$label = $payment_list[$val];
					   	    	if($credentials['card_fee']>=1){
					   	    		$label =  Yii::t("default","Stripe (card fee [fee])",array(
					         		  '[fee]'=>FunctionsV3::prettyPrice($credentials['card_fee'])
					         		));
					   	    	}					   	    
					   	    	$merchant_payment_list[]=array(
			...
			if (AddonMobileApp::isArray($merchant_payment_list)){			
				...
				if (FunctionsV3::hasModuleAddon('pointsprogram')){
					if (getOptionA('points_enabled')==1){
						...
						$points_balance=PointsProgram::getTotalEarnPoints($client_id,$mtid);
					}
				}
	...
	public function actionPlaceOrder()
	{
	    ...
	    /*CHECK ORDER SMS CODE*/
		if(AddonMobileApp::isOrderSmsEnabled($mtid)){
			$sms_code_validation=false;
			$token = isset($this->data['client_token'])?$this->data['client_token']:'';		
			$sms_code = isset($this->data['sms_code'])?$this->data['sms_code']:0;
			if(!empty($sms_code)){
				if(AddonMobileApp::validateOrderSMScode($sms_code,$token)){					
					$sms_code_validation=true;
				} else $this->msg = $this->t("You enter invalid sms code");
			} else $this->msg = $this->t("invalid sms code");
			
			if(!$sms_code_validation){
			   $this->output();	
			}
		}		
		/*END CHECK ORDER SMS CODE*/
		...
		if ($Validator->validate()){
			...
			switch ($this->data['payment_list']) {
				case "strip_ideal":
					...
					break;
				case "paypal_v2":
				    if ($fee_credentials=PaypalWrapper::getCredentials($mtid)){						
						if($fee_credentials['card_fee']>0.0001){
				...
				case "stp":
				    if ($fee_credentials=StripeWrapper::getCredentials($mtid)){						
						if($fee_credentials['card_fee']>0.0001){
							$this->data['card_fee']=$fee_credentials['card_fee'];
						}					
					}								
			    ...
			    case "mercadopago":
				    if ($fee_credentials=mercadopagoWrapper::getCredentials($mtid)){						
						if($fee_credentials['card_fee']>0.0001){
							$this->data['card_fee']=$fee_credentials['card_fee'];
						}					
					}								
					  ...
					  /* dump($params);
					   ...
					   if ( Yii::app()->functions->isMerchantCommission($mtid)){
					    ...
					    foreach ($cart['cart'] as $val_item) {
					    	...
					    	if(is_array($params_details['cooking_ref'])){
					    		$params_details['cooking_ref']='';
					    	}					    
					    	//dump($params_details);							    
					    ...
					    {					    	
	    					...
	    					case "stp":
	    						$this->details['next_step']='init_webview';
	    					    $this->details['redirect_url'] = websiteUrl()."/mobileapp/stripe/?id=".urlencode($order_id);
	    						break;		
	    					...
	    					case "paypal_v2":	    		    					    		
	    					    $this->details['next_step']='init_webview';
	    					    $this->details['redirect_url'] = websiteUrl()."/mobileapp/paypal/?id=".urlencode($order_id);
	    					    break;
	    					    
	    					case "vog":	    		    					    		
	    					    $this->details['next_step']='init_webview';
	    					    $this->details['redirect_url'] = websiteUrl()."/mobileapp/voguepay/?id=".urlencode($order_id);
	    					    break;    
	    					    
	    					case "btr":
	    						$this->details['next_step']='init_webview';
	    					    $this->details['redirect_url'] = websiteUrl()."/mobileapp/braintree/?id=".urlencode($order_id);
	    						break;
	    							    						
    						 case "mercadopago":
    						    $this->details['next_step']='init_webview';
    					        $this->details['redirect_url'] = websiteUrl()."/mobileapp/mercadopago/?id=".urlencode($order_id);
    						 break;
	    							    					    	    											    					
	    					default:	    					    
	    					    $this->msg = t("The payment options that you selected is not available at the moment");
	    					    $this->code=2;
	    					    break;
					    }
	...
	{		
		...
		if ( $Validator->validate()){
	    	...
	    	if($review_type==2) {	    		
	    	  ...
	    	  if(!is_array($order_info) && count( (array) $order_info)<=1){
	    	  	 $this->msg = $this->t("order id not found");
	    	  	 $this->output();
	    	  }	    	
	    		...
	    		if ( FunctionsV3::hasModuleAddon("pointsprogram")){	    				    			
	    			if($review_type==2){
	    			   if (method_exists('PointsProgram','addReviewsPerOrder')){
	    				   PointsProgram::addReviewsPerOrder($order_id,$client_id,$review_id,$mtid,$order_info['status']);
	    			   }	    			   
	    			} else {
		...
		if ( $res=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {
			...
			if($order){
				...
				foreach ($order as $val) {
					...
					if(method_exists('FunctionsV3','canCancelOrderNew')){						
						if(FunctionsV3::canCancelOrderNew($val['request_cancel'],$val['date_created'],
						$val['status'],$val['order_locked'],$val['request_cancel_status'],$cancel_order_enabled
						)){
					...
					$data[]=array(
					  ...
					  'place_on'=>Yii::t("mobile","Place on [date]",array('[date]'=> FunctionsV3::prettyDate($val['date_created'])) ),
					  'payment_type_orig'=>$val['payment_type'],
	...
	public function actionregisterUsingFb()
	{
		...
		$save_pic = getOptionA('mobile_fb_save_pic');
	...
	{		
		...
		/*$icons=array(
		  'from_icon'=>websiteUrl()."/protected/modules/mobileapp/assets/images/marker_orange_32.png",
		  'destination_icon'=>websiteUrl()."/protected/modules/mobileapp/assets/images/racing-flag.png",
		);	*/
		$icons=array(
		  'from_icon'=>websiteUrl()."/protected/modules/mobileapp/assets/images/marker_orange.png",
		  'destination_icon'=>websiteUrl()."/protected/modules/mobileapp/assets/images/marker_green.png",
		  'driver_icon'=>websiteUrl()."/protected/modules/mobileapp/assets/images/scooter.png",
		);	
	...
	//		    'location_accuracy'=>!empty(getOptionA('mobileapp_location_accuracy'))?getOptionA('mobileapp_location_accuracy'):'false',
			    ...
			    'enabled_delivery_select_map'=>getOptionA('enabled_map_selection_delivery'),
	...
	public function actionapplyRedeemPoints()
	{
		...
		if ( !$resp=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {
			$this->msg[]=AddonMobileApp::t("invalid token");
			 ...
			 $client_id=$resp['client_id'];
			 $balance_points=PointsProgram::getTotalEarnPoints($client_id,$merchant_id);			 
		...
		if ($Validator->validate()){
			...
			if(isset($this->data['cart_delivery_charges'])){
			   $cart_sub_total+=unPrettyPrice( (float) $this->data['cart_delivery_charges']);
			}
			if(isset($this->data['cart_packaging'])){
			   $cart_sub_total+=unPrettyPrice( (float) $this->data['cart_packaging']);
			}
					...
					if($task['driver_id']>0 && $continue==TRUE){
						...
						if (!empty($task['location_lat']) && !empty($task['location_lng']) 
							...
							if($task_distance_resp){
							   ...
							   if(is_array($task_distance_resp_raw) && count( (array) $task_distance_resp_raw)>=1){
	...
	{		
		if ( $client=AddonMobileApp::getClientTokenInfo($this->data['client_token'])){			
			...
			$params=array(
			  ...
			  'credit_card_number'=>FunctionsV3::maskCardnumber($p->purify($this->data['cc_number'])),
			  ...
			  'ip_address'=>$_SERVER['REMOTE_ADDR'],
			  'encrypted_card'=> CreditCardWrapper::encryptCard($p->purify($this->data['cc_number'])),
			);
	...
	public function actiongetCategory()
	{
		...
		$trans=getOptionA('enabled_multiple_translation'); 			
		if ( $trans==2 && isset($_GET['lang_id'])){
			$new=array(); $dish_list = array();			
			if(is_array($data['menu_category']) && count( (array) $data['menu_category'])>=1){
				foreach ($data['menu_category'] as $val) {	 					
					$val['category_name']=stripslashes($val['category_name']);
					$val['category_name']=AddonMobileApp::translateItem('category',
					$val['category_name'],$val['cat_id']);					
	...
	public function actiongetItem()
	{
		...
		if ($res=$DbExt->rst($stmt)){
			foreach ($res as $val) {
				...
				if (is_array($cooking_ref) && count( (array) $cooking_ref)>=1){
					$single_item=1;
				}				
				...
				$price_new=array();
				if (is_array($price)  && count( (array) $price)>=1){
					foreach ($price as $val_price) {
						...
						$val_price['size'] =  qTranslate($val_price['size'],'size', array(
					      'size_trans'=>$val_price['size_trans']
					    ));
				...
				if(isset($this->data['lang'])){					
					...
					if(is_array($item_name_trans) && count( (array) $item_name_trans)>=1){					   
	...
	public function actionGetCategoryList()
	{
		...
		if (is_array($new_category_list) && count( (array) $new_category_list)>=1){
			$this->code = 1; 
			$this->msg = "ok";
			$this->details = array(
    ...
    public function actionloadPhotos()
    {
    	...
    	if(is_numeric($merchant_id)){
            ...
            if(is_array($gallery) && count( (array) $gallery)>=1){
				 	...
				 	Yii::app()->functions->displayOrderHTML(array(
					  ...
					  switch ($data['trans_type']) {
					  	case "delivery":
					  		...
					  		$details[]=array('label'=>t("Delivery Date"),'value'=> FunctionsV3::prettyDate($data['delivery_date']) );
					  		$details[]=array('label'=>t("Delivery Time"),'value'=> FunctionsV3::prettyTime($data['delivery_time']) );
					    ...
					    case "pickup":
					  		...
					  		$details[]=array('label'=>t("Pickup Date"),'value'=> FunctionsV3::prettyDate($data['delivery_date']) );
					  		$details[]=array('label'=>t("Pickup Time"),'value'=> FunctionsV3::prettyTime($data['delivery_time']) );
					  		break;
					  	...
					  	case "dinein":
					  		...
					  		$details[]=array('label'=>t("Dine in Date"),'value'=> FunctionsV3::prettyDate($data['delivery_date']) );
					  		$details[]=array('label'=>t("Dine in Time"),'value'=> FunctionsV3::prettyTime($data['delivery_time']) );
					  		
					  		$details[]=array('label'=>t("Number of guest"),'value'=> !empty($data['dinein_number_of_guest'])?$data['dinein_number_of_guest']:'' );
					  		$details[]=array('label'=>t("Table Number"),'value'=> !empty($data['dinein_table_number'])?$data['dinein_table_number']:'' );
	...
	public function actiongetNotificationList()
	{
		...
		if(is_array($data) && count( (array) $data)>=1){
			...
			foreach ($data as $val) {
				...
				$val['push_title']= $this->t($val['push_title']);
				$new_data[]=$val;								
			}
			$this->code = 1;
			$this->msg ="OK";
	...
	{		
		$token = isset($this->data['client_token'])?$this->data['client_token']:'';
		$client=AddonMobileApp::getClientTokenInfo($token);
		if(!$client){
			$this->msg=$this->t("Token not valid");
			$this->output();
			Yii::app()->end();
		...
		$phone = isset($this->data['contact_phone'])?trim($this->data['contact_phone']):'';
		
		if(!empty($phone)){						
			if($merchant_id>=1){
				$sms_balance=Yii::app()->functions->getMerchantSMSCredit($merchant_id);
				if ( $sms_balance>=1){
					$code=FunctionsK::generateSMSOrderCode($phone);
					$sms_msg = Yii::t("mobile","Your order sms code is [code]",array(
					  '[code]'=>$code
					));
					
					$data=array();
					
					if ( $data = AddonMobileApp::getSMScodeByToken($token)){						
						$waiting_time_define=getOption($merchant_id,'order_sms_code_waiting');	
						if (!is_numeric($waiting_time_define)){
							$waiting_time_define=5;
						}									
						$time_1=date('Y-m-d g:i:s a');
						$time_2=$data['date_created'];						
						$time_diff=Yii::app()->functions->dateDifference($time_2,$time_1);											
						...
						if (is_array($time_diff) && count($time_diff)>=1){
							if ($time_diff['days']==0 && $time_diff['hours']==0){
								if ($time_diff['minutes']<$waiting_time_define){
										...
										$this->msg = Yii::t("mobile","Spam protection. you cannot request another order sms code in less than [minutes] Minutes",array(
					...
					if ( $resp=Yii::app()->functions->sendSMS($phone,$sms_msg)){
						if ($resp['msg']=="process"){
							$this->code=1;				    	    
				    	    $this->msg = Yii::t("mobile","Your order sms code has been sent to [phone]",array(
				    	      '[phone]'=>$phone
				    	    ));
					    	...
					    	if (is_array($data) && count( (array) $data)>=1){
					    		$db->updateData("{{order_sms}}",$params,'id',$data['id']);
					    	} else $db->insertData("{{order_sms}}",$params);
						...
						} else $this->msg = $this->t("Sorry but we cannot sms code this time")." ".$resp['msg'];
					} else $this->msg = $this->t("Sorry but we cannot sms code this time");
				} else $this->msg = $this->t("Sorry but this merchant does not have enought sms credit to send sms");		
			} else $this->msg = $this->t("invalid merhant id");
		} else $this->msg = $this->t("invalid mobile number");
		$this->output();
	}
	
	public function actionvalidateOrderSMS()
	{
		$token = isset($this->data['client_token'])?$this->data['client_token']:'';
		$client=AddonMobileApp::getClientTokenInfo($token);
		if(!$client){
		...
		if(!empty($sms_code)){			
			if(AddonMobileApp::validateOrderSMScode($sms_code,$token)){				
				$this->code=1;
				$this->msg = "OK";
				$this->details = $this->data['next_step_after_sms'];
			} else $this->msg = $this->t("You enter invalid sms code");
		} else $this->msg = $this->t("invalid sms code");
	...
	public function actioncheckOrderSMSSettings()
	{		
		$this->data = $_POST;
		$merchant_id = isset($this->data['merchant_id'])?$this->data['merchant_id']:0;
		if($merchant_id>=1){
			if (AddonMobileApp::isOrderSmsEnabled($merchant_id)){
				$this->code = 1;
				$this->msg = "enabled";
			} else $this->msg = "not enabled";
		} else $this->msg = $this->t("Invalid merchant id");
		$this->output();
	}
	...
	public function actionsearchByCategory()
	{
		...
		if($merchant_id>0){
			...
			AND merchant_id = ".FunctionsV3::q($this->data['merchant_id'])."
			LIMIT 0,10
			";						
			if($res = $db->rst($stmt)){
				$data = array();
				foreach ($res as $val) {
					...
					$category_name = AddonMobileApp::highlight_word($category_name_trans,$search_str);
			...
			if($merchant_id>0){
				...
				if ($res = $db->rst($stmt)){
					...
					foreach ($res as $val) {
					    ...
					    $item_name = AddonMobileApp::highlight_word($item_name_trans,$search_str);
